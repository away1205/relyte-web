...
function Header() {
  ...
  return (
    <header
      className='fixed bg-white w-screen border-b border-[#e0e0e0] z-40 px-4 sm:px-10'
      id='header'
      ref={containerRef}
    >
      <div className='flex justify-between items-center max-screen mx-auto py-6'>
        ...
        <MagneticAnim>
          <button
            className='md:py-1 md:px-4 lg:px-6 lg:py-2 border-black border rounded-3xl max-md:hidden'
            onClick={() => {
              gsap.to(window, {
                duration: 1,
        ...
        <div className='md:hidden relative'>
          ...
          <div className='md:hidden absolute top-0 -right-8 h-10 w-10 rounded-xl bg-black-100 opacity-5 header-anim px-4 py-4 flex flex-col'>
            ...
            <div className='mt-auto mb-10'>
              <div className='text-white mb-16'>
                <ContactAddress color='text-[white]' />
              </div>
              <LinkSocial />
            </div>
          </div>
  ...
  return (
    <ul
      className={` md:flex ${
        !isSmallScreen
          ? 'hidden gap-14'
          : 'text-[white] flex flex-col text-[3.75rem] leading-none gap-4 font-light uppercase'
      }`}
    >
      {navList.map((item) => {
        return (
          <MagneticAnim key={item.value}>
            <li className='cursor-pointer h-fit'>
              <a
                // href={`#${item.value}`}
                onClick={() => {
                  gsap.to(window, {
